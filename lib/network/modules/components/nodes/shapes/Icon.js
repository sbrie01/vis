...
class Icon extends NodeBase {
	...
	if (this.options.icon.code !== undefined) {
		...
		let padding = (this.options.icon.background.padding ? this.options.icon.background.padding : 0);
		
		ctx.beginPath();
		ctx.fillStyle = this.options.icon.background.color || "rgba(0,0,0,0)";
		
		let xCoord = x - ((iconSize + padding) / 2);
		let yCoord = y - ((iconSize + padding) / 2)
		let sideLength = iconSize + padding;
